...
module('Integration | Component | map', function(hooks) {
  ...
  test('it renders a map for the specified parameters', async function(assert) {
    ...
    assert.ok(src.includes('-122.4184,37.7797'), 'the src should include the lng,lat,zoom parameter');
  ...
  test('it updates the `src` attributes when the arguments change', async function(assert) {
    this.setProperties({
      lat: 37.7797,
      lng: -122.4184,
      zoom: 10,
    ...
    assert.ok(img.src.includes('-122.4184,37.7797'), 'the src should include the lng,lat,zoom parameter');
